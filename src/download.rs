...
impl DownloadArguments {
    pub fn clone_repository(&mut self) -> Result<Repository, Box<dyn std::error::Error>> {
        ...
        let working_directory = working_directory_buffer.join(path_to_join);

        let repository = RepoBuilder::new().clone(&url, &working_directory)?;
    ...
    pub fn extract_lfs_urls(
        ...
        debug!("Trying to locate filepath at: {:?}", &repository_path);
        ...
        for lfs_file in lfs_files {
            ...
            if let Some(oid) = oid {
                let url = format!(
                    "{}/{}/resolve/main/{}",
                    base_url, self.repository, &lfs_file
                );
    ...
    async fn download_single_file_resume(
        ...
        debug!("Resume from position: {}", &start);
        ...
        debug!("Headers when retrying: {:?}", headers);

        // streaming logic
        if status {
            let mut content = retry.bytes_stream();
    ...
    async fn download_single_file(
        ...
        let total_filesize = response.content_length().ok_or("Failed to get filesize")?;

        // construct the eventual filepath,
        // make this file downloaded to the repository folder
        let filepath = std::path::Path::new(&repository_local_path).join(filename);

        // create a file for writing data
        let mut file = std::fs::File::create(filepath)?;

        // initiate the progress bar
        let pb = progress_bar.add(indicatif::ProgressBar::new(total_filesize));
        pb.set_style(
        ...
        if result_hash == expected_sha256 {
            info!("SHA256 hash verification succeeded for file {}", filename);
        } else {
            warn!(
    ...
    pub async fn download_files(
        ...
        if large_file_information.len() == 0 {
            warn!("No LFS urls had been detected. This could potentially be an error?");
        }

        let progress_bar = std::sync::Arc::new(indicatif::MultiProgress::new());
        let mut handlers = Vec::new();
        let semaphore = std::sync::Arc::new(tokio::sync::Semaphore::new(self.tasks));

        for single_large_file_information in large_file_information {
            let client_in_thread = client.clone();
            let repository_local_path_thread = self.repository_local_path.clone().unwrap();
            let progress_bar_thread = progress_bar.clone();
            let semaphore_thread = semaphore.clone();

            let handler = tokio::task::spawn(async move {
                let _permit = semaphore_thread.acquire().await.unwrap();
