...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Concurrency is sat to: {}", &arguments.tasks);

    // handle the case in which the repo exists locally
    if std::path::Path::new(&arguments.repository).exists() {
        println!("Repository has detected in the current working directory, overwrite?(Y/N)");
    ...
    match arguments.clone_repository() {
        Ok(result) => {
            match arguments.read_lfs_pointers(&result.path().parent().unwrap().to_path_buf()) {
                Ok(lfs_files) => match arguments.extract_lfs_urls(
                    &result.path().parent().unwrap().to_path_buf(),
                    lfs_files,
                    &ensure_trailing_slash(
                        option_env!("HF_ENDPOINT").unwrap_or("https://hf-mirror.com/"),
                    ),
                ) {
                    Ok(large_file_information) => {
                        arguments.download_files(large_file_information).await?
                    }
                    Err(error) => error!("Downloading large files failed due to {}", error),
                },
                Err(error) => error!("Extracting LFS urls failed due to {}", error),
            };

            info!(
                "Git cloned the repository to: {}",
                result
                    .path()
                    .parent()
                    .unwrap()
        ...
        Err(error) => error!("Git clone has failed {}", error),
    };
